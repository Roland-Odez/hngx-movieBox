        ...
        <div className='relative'>
            ...
            <Link href={`/movies/${props.data.id}`} className='flex flex-col rounded-md gap-y-2 md:gap-y-3 md:hover:scale-105 transition-[transform] duration-700 hover:shadow-md'>
                <div className=' object-cover'>
                    <Image src={`https://image.tmdb.org/t/p/w220_and_h330_face/${props.data.poster_path}`} alt='movie post' width={486} height={720} className='w-ful h-full block rounded-t-md object-cover' />
                </div>
                <div className='flex flex-col gap-y-2 md:gap-y-3 p-2'>
                    <p className='text-xs font-bold text-gray-400'>{`${movie.production_countries[0].iso_3166_1}, ${props.data.release_date.split('-')[0]} - Current`}</p>
                    <p className='text-[18px] font-bold text-gray-900'>{props.data.title}</p>
